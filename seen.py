...
def extract_exif(image_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"⚠️  Viga faili {image_path} lugemisel: {e}", file=sys.stderr)
...
def move_to_trash(filepath: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️  Hoiatus: Ei saanud faili prügikasti liigutada: {e}", file=sys.stderr)
            return False
...
def main():
        ...
        if fp.lower().endswith(".zip") and zipfile.is_zipfile(fp):
            ...
            try:
                ...
                if extracted_imgs:
                    ...
                    print(f"⚠️  Arhiivist '{os.path.basename(fp)}' ei leitud ühtegi toetatud pildifaili!", file=sys.stderr)
            except Exception as e:
                print(f"⚠️  Viga ZIP arhiivi lahtipakkimisel ({fp}): {e}", file=sys.stderr)
        else:
    ...
    if primary.get("lat") and primary.get("lon"):
        ...
        print("⚠️  Hoiatus: Fotost ei leitud GPS koordinaate!")
    ...
    if not flags.get("keep"):
        for fp in original_files_to_trash:
            if fp and os.path.exists(fp):
                if move_to_trash(fp):
                    print(f"🗑️  '{os.path.basename(fp)}' liigutati prügikasti (Downloads kaust on puhas).")
